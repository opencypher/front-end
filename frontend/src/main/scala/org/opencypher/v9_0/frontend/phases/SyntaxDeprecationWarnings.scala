...
import org.opencypher.v9_0.rewriting.rewriters.replaceAliasedFunctionInvocations.renamedFunctions
...
object SyntaxDeprecationWarnings extends VisitorPhase[BaseContext, BaseState] {
  ...
  private def findDeprecations(statement: Statement): Set[InternalNotification] =
    statement.treeFold(Set.empty[InternalNotification]) {
      case f@FunctionInvocation(_, FunctionName(name), _, _) if renamedFunctions.contains(name) =>
        seq => (seq + DeprecatedFunctionNotification(f.position, name, renamedFunctions(name)), None)
