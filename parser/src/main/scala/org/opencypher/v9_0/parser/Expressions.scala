...
trait Expressions extends Parser
  ...
  private def Expression7: Rule1[org.opencypher.v9_0.expressions.Expression] = rule("an expression") {
    Expression6 ~ zeroOrMore(WS ~ (
      group(operator("=~") ~~ Expression6) ~~>> (ast.RegexMatch(_: org.opencypher.v9_0.expressions.Expression, _))
        | group(keyword("IN") ~~ Expression6) ~~>> (ast.In(_: org.opencypher.v9_0.expressions.Expression, _))
        | group(keyword("STARTS WITH") ~~ Expression6) ~~>> (ast.StartsWith(_: org.opencypher.v9_0.expressions.Expression, _))
        | group(keyword("ENDS WITH") ~~ Expression6) ~~>> (ast.EndsWith(_: org.opencypher.v9_0.expressions.Expression, _))
        | group(keyword("CONTAINS") ~~ Expression6) ~~>> (ast.Contains(_: org.opencypher.v9_0.expressions.Expression, _))
  ...
  private def Expression6: Rule1[org.opencypher.v9_0.expressions.Expression] = rule("an expression") {
    Expression5 ~ zeroOrMore(WS ~ (
      group(operator("+") ~~ Expression5) ~~>> (ast.Add(_: org.opencypher.v9_0.expressions.Expression, _))
        | group(operator("-") ~~ Expression5) ~~>> (ast.Subtract(_: org.opencypher.v9_0.expressions.Expression, _))
      ))
  }

  private def Expression5: Rule1[org.opencypher.v9_0.expressions.Expression] = rule("an expression") {
    Expression4 ~ zeroOrMore(WS ~ (
      group(operator("*") ~~ Expression4) ~~>> (ast.Multiply(_: org.opencypher.v9_0.expressions.Expression, _))
        | group(operator("/") ~~ Expression4) ~~>> (ast.Divide(_: org.opencypher.v9_0.expressions.Expression, _))
        | group(operator("%") ~~ Expression4) ~~>> (ast.Modulo(_: org.opencypher.v9_0.expressions.Expression, _))
      ))
  }

  private def Expression4: Rule1[org.opencypher.v9_0.expressions.Expression] = rule("an expression") {
    Expression3 ~ zeroOrMore(WS ~ (
      group(operator("^") ~~ Expression3) ~~>> (ast.Pow(_: org.opencypher.v9_0.expressions.Expression, _))
      ))
  }
