...
sealed trait ReadAdministrationCommand extends AdministrationCommand {
  ...
  override def semanticCheck: SemanticCheck = (initialState: SemanticState) => {
    ...
    def checkProjection(r: ProjectionClause, prevErrors: Seq[SemanticErrorDef]): SemanticCheck = (state: SemanticState) => {
      val closingResult =
        (r.semanticCheck chain r.where.map(checkForExistsSubquery).getOrElse(None) chain checkForReturnPattern).run(state)
      val continuationResult = r.semanticCheckContinuation(closingResult.state.currentScope.scope).run(closingResult.state)
    ...
    Seq(yields, returns).foldLeft(initialCheckResult) { (checkResult, maybeClause) =>
      maybeClause match {
        ...
        case Some(r: ProjectionClause) =>
          checkProjection(r, checkResult.errors).chain(recordCurrentScope(r)).run(checkResult.state)
      }
    }
  }
}
