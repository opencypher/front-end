...
import org.opencypher.v9_0.ast.{Access, ActionResource, AdministrationCommand, AliasedReturnItem, AllDatabasesQualifier, AllDatabasesScope, AllGraphsScope, AllLabelResource, AllNodes, AllPropertyResource, AllQualifier, AllRelationships, AlterDatabase, AlterDatabaseAlias, AlterUser, AscSortItem, Clause, ConstraintVersion, ConstraintVersion0, ConstraintVersion2, Create, CreateBtreeNodeIndex, CreateBtreeRelationshipIndex, CreateDatabase, CreateDatabaseAlias, CreateFulltextNodeIndex, CreateFulltextRelationshipIndex, CreateIndexOldSyntax, CreateLookupIndex, CreateNodeKeyConstraint, CreateNodePropertyExistenceConstraint, CreatePointNodeIndex, CreatePointRelationshipIndex, CreateRangeNodeIndex, CreateRangeRelationshipIndex, CreateRelationshipPropertyExistenceConstraint, CreateRole, CreateTextNodeIndex, CreateTextRelationshipIndex, CreateUniquePropertyConstraint, CreateUser, CurrentUser, DatabasePrivilege, DatabaseScope, DbmsPrivilege, DefaultDatabaseScope, DefaultGraphScope, Delete, DenyPrivilege, DescSortItem, DestroyData, DropConstraintOnName, DropDatabase, DropDatabaseAlias, DropIndex, DropIndexOnName, DropNodeKeyConstraint, DropNodePropertyExistenceConstraint, DropRelationshipPropertyExistenceConstraint, DropRole, DropUniquePropertyConstraint, DropUser, DumpData, ElementQualifier, ElementsAllQualifier, ExecutableBy, Foreach, FunctionAllQualifier, FunctionQualifier, GrantPrivilege, GrantRolesToUsers, GraphAction, GraphPrivilege, GraphScope, GraphSelection, HasCatalog, Hint, HomeDatabaseScope, HomeGraphScope, IfExistsDo, IfExistsDoNothing, IfExistsInvalidSyntax, IfExistsReplace, IfExistsThrowError, LabelAllQualifier, LabelQualifier, LabelsResource, Limit, LoadCSV, Match, Merge, MergeAction, NamedDatabaseScope, NamedGraphScope, NoOptions, NodeByIds, NodeByParameter, OnCreate, OnMatch, Options, OptionsMap, OptionsParam, OrderBy, PrivilegeQualifier, ProcedureAllQualifier, ProcedureQualifier, ProcedureResult, ProcedureResultItem, ProjectingUnionAll, ProjectingUnionDistinct, PropertiesResource, PropertyResource, Query, QueryPart, ReadOnlyAccess, ReadWriteAccess, RelationshipAllQualifier, RelationshipByIds, RelationshipByParameter, RelationshipQualifier, Remove, RemoveHomeDatabaseAction, RemoveLabelItem, RemovePropertyItem, RenameRole, RenameUser, Return, ReturnItem, ReturnItems, RevokePrivilege, RevokeRolesFromUsers, SchemaCommand, SeekOnly, SetClause, SetExactPropertiesFromMapItem, SetHomeDatabaseAction, SetIncludingPropertiesFromMapItem, SetLabelItem, SetOwnPassword, SetPropertyItem, SetPropertyItems, ShowAllPrivileges, ShowConstraintsClause, ShowCurrentUser, ShowDatabase, ShowFunctionsClause, ShowIndexesClause, ShowPrivilegeCommands, ShowPrivilegeScope, ShowPrivileges, ShowProceduresClause, ShowRoles, ShowRolesPrivileges, ShowTransactionsClause, ShowUserPrivileges, ShowUsers, ShowUsersPrivileges, SingleQuery, Skip, Start, StartDatabase, Statement, StopDatabase, SubqueryCall, TerminateTransactionsClause, UnaliasedReturnItem, Union, UnionAll, UnionDistinct, UnresolvedCall, Unwind, UseGraph, User, UserAllQualifier, UserQualifier, UsingAnyIndexType, UsingBtreeIndexType, UsingHint, UsingIndexHint, UsingIndexHintType, UsingJoinHint, UsingScanHint, UsingTextIndexType, Where, With, Yield, YieldOrWhere}
...
case class Prettifier(
  ...
  def asString(command: SchemaCommand): String = {
    ...
    val commandString = command match {
      ...
      case CreateNodePropertyExistenceConstraint(Variable(variable), LabelName(label), property, name, ifExistsDo, options, containsOn, constraintVersion, _) =>
        val startOfCommand = getStartOfCommand(name, ifExistsDo, "CONSTRAINT")
      ...
      case CreateRelationshipPropertyExistenceConstraint(Variable(variable), RelTypeName(relType), property, name, ifExistsDo, options, containsOn, constraintVersion, _) =>
        val startOfCommand = getStartOfCommand(name, ifExistsDo, "CONSTRAINT")
        val propertyString = propertyToStringExistenceConstraint(property, constraintVersion)
        val optionsString = asString(options)
        val forOrOn = if (containsOn) "ON" else "FOR"
