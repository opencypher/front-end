...
class ToBooleanListTest extends FunctionTestBase("toBooleanList")  {
  ...
  test("shouldAcceptCorrectTypes") {
    ...
    testValidTypes(CTList(CTString))(CTList(CTBoolean))
    testValidTypes(CTList(CTFloat))(CTList(CTBoolean))
    testValidTypes(CTList(CTInteger))(CTList(CTBoolean))
    testValidTypes(CTList(CTPoint))(CTList(CTBoolean))
  }
  ...
  test("shouldFailIfWrongNumberOfArguments") {
    testInvalidApplication()(
      "Insufficient parameters for function 'toBooleanList'"
    )
    testInvalidApplication(CTString, CTString)(
      "Too many parameters for function 'toBooleanList'"
    )
  }
}
