...
class ProjectNamedPathsTest extends CypherFunSuite with AstRewritingTestSupport with TestName {
  ...
  test("CALL {MATCH p = (a) RETURN p} RETURN p" ) {
    ...
    val CALL = {
      val MATCH = Match(optional = false,
        Pattern(List(
          EveryPath(
            NodePattern(Some(a), List(), None, None, None)(pos))
        ))(pos), List(), None)(pos)
    ...
    val expected: Query = Query(SingleQuery(List(CALL, RETURN))(pos))(pos)
      ...
      Pattern(List(
        EveryPath(
          RelationshipChain(
            ...
            NodePattern(Some(b), List(), None, None, None)(pos)
          )(pos))
      ))(pos), List(), None)(pos)
      ...
      Pattern(List(
        ...
        EveryPath(
          RelationshipChain(
            ...
            NodePattern(Some(c), List(), None, None, None)(pos)
          )(pos))
      ))(pos), List(), None)(pos)
  ...
  test("MATCH p = (a), (b) CALL {WITH p, b RETURN 1 AS one} RETURN p") {
    ...
    val MATCH = Match(optional = false,
      Pattern(List(
        EveryPath(NodePattern(Some(a), List(), None, None, None)(pos)),
        EveryPath(NodePattern(Some(b), List(), None, None, None)(pos)),
      ))(pos), List(), None)(pos)
    ...
    val MATCH =
      Match(optional = false,
        Pattern(List(
          EveryPath(
            RelationshipChain(
              NodePattern(Some(aId), List(), None, None, None)(pos),
              RelationshipPattern(Some(rId), List(), None, None, None, SemanticDirection.OUTGOING)(pos), NodePattern(Some(bId), List(), None, None, None)(pos)
            )(pos))
        ))(pos), List(), None)(pos)
