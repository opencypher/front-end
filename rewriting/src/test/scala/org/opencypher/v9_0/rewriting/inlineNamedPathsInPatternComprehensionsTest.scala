...
class inlineNamedPathsInPatternComprehensionsTest extends CypherFunSuite with AstConstructionTestSupport {
  ...
  test("does not touch comprehensions without named path") {
    val input: ASTNode = PatternComprehension(None, RelationshipsPattern(RelationshipChain(NodePattern(None, Seq.empty, None, None) _, RelationshipPattern(None, Seq.empty, None, None, None, SemanticDirection.OUTGOING) _, NodePattern(None, Seq.empty, None, None) _) _)_, None, literalString("foo"))(pos, Set.empty, "", "")
  ...
  test("removes named path if not used") {
    val input: PatternComprehension = PatternComprehension(Some(varFor("p")), RelationshipsPattern(RelationshipChain(NodePattern(Some(varFor("a")), Seq.empty, None, None) _, RelationshipPattern(Some(varFor("r")), Seq.empty, None, None, None, SemanticDirection.OUTGOING) _, NodePattern(Some(varFor("b")), Seq.empty, None, None) _) _)_, None, literalString("foo"))(pos, Set.empty, "", "")
